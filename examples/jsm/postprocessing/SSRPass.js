...
var SSRrPass = function ({
	...
	Object.defineProperty(this, "isBouncing", {
		...
		set(val) {
			...
			if (val) {
				this.ssrMaterial.uniforms[
					"tDiffuse"
				].value = this.prevRenderTarget.texture;
			} else {
				this.ssrMaterial.uniforms[
					"tDiffuse"
	...
	if (this.isSelective) {
		...
		this.ssrMaterial.uniforms[
			"tMetalness"
		].value = this.metalnessRenderTarget.texture;
	}
	this.ssrMaterial.uniforms[
		"tDepth"
			...
			case SSRrPass.OUTPUT.Default:
				if (this.isBouncing) {
					...
					if (this.isBlur)
						this.copyMaterial.uniforms[
							"tDiffuse"
						].value = this.blurRenderTarget2.texture;
					else
						this.copyMaterial.uniforms[
							"tDiffuse"
						].value = this.ssrRenderTarget.texture;
